...
# http://0.0.0.0:8080/api_alumnos?user_hash=12345&action=put&id_alumno=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=0
    def put(self, nombre,domicilio,telefono,maestria):
        try:
            config.model.insert_alumnos(nombre,domicilio,telefono,maestria)
            alumnos_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(alumnos_json)
        except Exception as e:
...
# http://0.0.0.0:8080/api_alumnos?user_hash=12345&action=update&id_alumno=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=default.jpg
    def update(self, id_alumno, nombre,domicilio,telefono,maestria):
        try:
            config.model.edit_alumnos(id_alumno,nombre,domicilio,telefono,maestria)
            alumnos_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(alumnos_json)
        except Exception as e:
        ...
        try:
            ...
            if user_hash == '12345':
                if action is None:
                    ...
                    return self.put(nombre,domicilio,telefono,maestria)
                elif action == 'delete':
                    return self.delete(id_alumno)
                elif action == 'update':
                    return self.update(id_alumno, nombre,domicilio,telefono,maestria)
            else:
                raise web.seeother('/404')
        except Exception as e:
